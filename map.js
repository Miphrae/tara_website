...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const lookup = [];  // will hold { name, marker }
  
    // A qualitative color palette with high contrast entries
    const borderPalette = [
      '#1f77b4', // blue
      '#ff7f0e', // orange
      '#2ca02c', // green
    ...
    // Load & style borders.kml
    omnivore.kml('/borders.kml', null, L.geoJson(null, {
      style: feature => {
        // use feature.properties.name (or fallbacks) as the key
    ...
    // Pin icon factory (unchanged from before)
    ...
    // Load points.csv and add markers (unchanged)
    fetch('/points.csv')
      .then(res => res.text())
